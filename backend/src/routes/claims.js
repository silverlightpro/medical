...
router.post('/:id/generate-questions', async (req, res, next) => {
  try {
    ...
    const claim = await prisma.claim.update({ where: { id: req.params.id }, data: { analysisQuestions: JSON.stringify(questions) } });
    res.json({ questions });
  } catch (e) { next(e); }
});
...
router.post('/:id/identify-events', async (req, res, next) => {
  try {
    ...
    await prisma.claim.update({ where: { id: req.params.id }, data: { potentialClaimEvents: JSON.stringify(events) } });
    res.json({ events });
  } catch (e) { next(e); }
});
...
router.post('/:id/generate-final-doc', async (req, res, next) => {
  ...
  await prisma.claim.update({ where: { id: req.params.id }, data: { finalDocument: JSON.stringify(finalDoc), status: 'Final Document Ready' } });
...
router.post('/:id/generate-va-form', async (req, res, next) => {
  try {
    ...
    await prisma.claim.update({ where: { id: req.params.id }, data: { vaFormData: JSON.stringify(vaFormData) } });
    res.json({ vaFormData });
  } catch (e) { next(e); }
});
